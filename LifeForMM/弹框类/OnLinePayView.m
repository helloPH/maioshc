...
-(void)reloadDataForVerification
{
    ...
    self.danhao = [NSString stringWithFormat:@"%@%@",userShequ_id,dateTime];
    ...
    self.body = [NSString stringWithFormat:@"妙生活城+%@%@",userName_tel,suffix];
}
...
{
    MyLog(@"-- 支付宝");
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"shequid":@"0"}];
...
-(void)wxPay
{
    if (wxPaymessage.count == 0) {
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"total_fee":self.payMoney,@"shequid":userShequ_id,@"body":self.body,@"userid":user_id}];
