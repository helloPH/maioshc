...
#import "Header.h"
@interface findPasViewController ()<UIGestureRecognizerDelegate,UIAlertViewDelegate,MBProgressHUDDelegate,UITextFieldDelegate>
{
...
-(void)telSureBtnAction
{
    ...
    BOOL isMatch = [phoneTest evaluateWithObject:tel.text];
    if(!isMatch){
...
-(void)changeNewPasSure
{
    ...
    if ([onePas.text isEqualToString:secPas.text]) {
        ...
        [HTTPTool getWithUrl:@"backPassword.action" params:pram success:^(id json) {
            NSDictionary *dic = (NSDictionary *)json;
            if ([[dic valueForKey:@"massage"]intValue]==1) {
                for (int i = 0; i<findWayNameAry.count; i++) {
                    UIView *chooseView =[findWayView viewWithTag:[findWayTagAry[i] integerValue]];
...
-(void)sendEmailAccess
{
    ...
    mbHud.delegate = self;
    [mbHud show:YES];
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"tel":userName_tel}];
