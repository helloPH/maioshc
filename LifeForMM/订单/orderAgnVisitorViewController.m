//
//  orderAgnVisitorViewController.m
//  LifeForMM
//
...
@interface orderAgnVisitorViewController ()<UIGestureRecognizerDelegate,UITableViewDataSource,UITableViewDelegate,UITextViewDelegate,PopViewDelegate,UITextViewDelegate,addressDelegate,MBProgressHUDDelegate,UITextFieldDelegate,newAddresPopDelegate,SelectedTimeViewDelegate>
{
    ...
    NSString *shouhuodizhi,*shouhuoren,*dianhua; //收货地址 收货人 电话
...
@end
@implementation orderAgnVisitorViewController
...
-(void)changeRelodMoey:(NSString *)adresId
{
    ...
    agnAdresModel *model = dataArray[0];
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id,@"dindan.danhao":_danhao,@"shequid":model.shequid,@"shdzid":adresId}];
    [HTTPTool getWithUrl:@"dingdanjine.action" params:pram success:^(id json) {
        NSDictionary *dic = (NSDictionary *)json;
...
{
    onlinePayWayPop = [[UIView alloc]initWithFrame:CGRectMake(kDeviceWidth*3/20.0, 330*MCscale, kDeviceWidth*7/10.0, 80*MCscale)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == orderTableView){
        ...
        if (indexPath.section == 0) {
            ...
            if (indexPath.row == lastChoosePay-1) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == orderTableView){
        if (indexPath.section == 0) {
            [self choosePayAction:indexPath.row+1];
        }
    }
    else{
...
{
    agnAdresModel *model = dataArray[0];
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"shequid":model.shequid}];
...
-(void)goToCheck
{
    ...
    if ([nameL.text isEqualToString:@""] || [telL.text isEqualToString:@""] || [adresL.text isEqualToString:@""]) {
        ...
        ppmdic = [NSMutableDictionary dictionaryWithDictionary:@{@"userid":user_id,@"shequid":model.shequid,@"dipuids":dianpuidStr,@"dindan.shouhuoren":nameL.text,@"dindan.tel":telL.text,@"dindan.shouhuodizhi":adresL.text,@"dindan.zhifufangshi":[NSString stringWithFormat:@"%ld",(long)lastChoosePay],@"dindan.youhui":youhuiStr,@"dindan.yingfujine":xiaojiStr,@"dindan.yuyuesongdadate":dingdanTimeStr,@"dindan.fudongfei":fujiafeiStr,@"dindan.dindanbeizhu":beizhuStr,@"dindan.fapiaotaitou":fapiaotouStr,@"dindan.shuliang":shuliangStr,@"dindan.youhuijuan":@"0",@"dindan.peisongfei":peisongfeiStr,@"dindan.danhao":_danhao,@"shdzid":defaultdizId,@"dindan.caigouchengben":caiGouMoneyStr}];
        //支付宝
        if (lastChoosePay == 2){
            [allBub hide:YES];
            if (!isWx) {
...
-(void)maskDisMiss
{
    if (popViewTag == 101 || popViewTag == 105 || popViewTag == 107 || popViewTag ==110) {
...
//选择地址
-(void)changeAddressAction:(UITapGestureRecognizer *)tap
{
    NSInteger btnTag = tap.view.tag;
    if (btnTag!=lastChooseAddress) {
        UIView *view = [changeAddressPopView viewWithTag:btnTag];
        UIImageView *newImage = (UIImageView *)[view viewWithTag:11];
        newImage.image = [UIImage imageNamed:@"选中"];
        UIView *lastView = [changeAddressPopView viewWithTag:lastChooseAddress];
        UIImageView *lastImage = (UIImageView *)[lastView viewWithTag:11];
        lastImage.image = [UIImage imageNamed:@"选择"];
        lastChooseAddress = btnTag;
    }
    userAddressModel *model = addresDataAry[btnTag-100];
    NSString *shouhuodz = [NSString stringWithFormat:@"%@",model.shouhuodizhi];
    UILabel *name = headViewsAry[0];
    UILabel *tel = headViewsAry[1];
    UILabel *adres = headViewsAry[2];
    name.text = model.name;
...
-(void)popBtnAction:(PopView *)pop atIndex:(NSInteger)index btnTag:(NSInteger)btag
{
    ...
    if (index != 106){
        ...
        //            UITextView *textView = newAddViewsAry[2];
        //            [self searchTipsWithKey:textView.text];
        //        }
